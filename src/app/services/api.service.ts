...
export class ApiService {
  ...
  searchByTypingGames(params: any): Observable<any> {
    ...
    if (params.authorsId)
      httpParams = httpParams.set('authorsId', params.authorsId.toString()); // con conversione in stringa che poi vedremo se necessaria
    if (params.categoriesId)
      httpParams = httpParams.set(
        'categoriesId',
        params.categoriesId.toString()
      );
    if (params.editorId)
      httpParams = httpParams.set('editorId', params.editorId.toString());
    return this.http.get(this.url + 'public/games/searchByTyping', {
      params: httpParams,
    });
  }
  ...
  SearchByTypingUser(id: number) {
    return this.http.get(
      this.url + 'admin/users/searchByTyping?id=' + id + '&active=true'
    );
  }
